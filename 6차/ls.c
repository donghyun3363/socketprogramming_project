...
{	
	...
	int socket_fd, client_fd, pre_client_fd;	// file des variable
	...
	listen(socket_fd, 10);	// listen status
	...
	while(1){
		...
		while(len_out = read(client_fd, buf, BUFFSIZE) >0){ 
			...
			Fflag=0;
			////////////////////////////////////////////			
			write(STDOUT_FILENO, buf, len_out);	// request message to stdout
			//////////////////////////////////////////// path decision			
			strtok(buf, " ");
			temp=strtok(NULL, " ");
