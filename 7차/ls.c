...
int client_pid[50];							// client of pid arr
int client_port[50];						// client of port arr
int client_time[50];						// server-client time arr
char *client_ip;							// client_ip saving variable
int idx=0;									// arr of all index
...
{	
	DIR *dirp;									// Dir struct
	struct dirent *dir;
	int len, len_out;							// write length of return variable
	int opt;
    ...
    signal(SIGCHLD, sig_handler);				// sigchld occur
	...
	strcpy(argv[0], "ls");
	strcpy(argv[1], "-al");
	memset(client_pid, 0, 50);
	memset(client_port, 0, 50);
	memset(client_time, 0, 50);
	////////////////////////////////////////////////////
	...
	if(listen(socket_fd, 5)<0){	// listen status
		printf("listen error\n");
	}
	...
	while(1){
		...
		////////////////////////////////////////////////
		cur_time=time('\0');		// current_time save 
		client_time[idx]=cur_time;	// saving to arr
		...
		if(pid<0){									// error
			...
			exit(0);								// client disconnect!!!
		}//------------------------------parent------------------------------
		else{
       		client_pid[idx]=pid;					// client pid save to arr
			client_port[idx]=client_addr.sin_port;	// client port save to arr
			idx++;									// idex increase
			//////////////////////////////////////////// connect message
			...
			for(i=0 ; i<idx ; i++){
				if(client_pid[i]!=0){
					...
					printf("%d	", client_port[i]);
					printf("%d \n", (int)cur_time-client_time[i]);	// current time - client time arr value
				}
			}
            printf("\n");
			////////////////////////////////////////////
...
void sig_handler(int sig)
{
	...
	time_t cur_time;			// current time struct
	int i=0;					// idx
	cur_time=time('\0');		// current time save
	if(sig == SIGCHLD){			// occur SIGCHLD signal
		pid = wait(NULL);		// child clean
		////////////////////////////////////////////////
		printf(" %d child process is terminated!\n", pid);
		//////////////////////////////////////////////// disconnect message	
		printf("[%d:%d] client was disconnected. \n", client_addr.sin_addr.s_addr, client_addr.sin_port);
		client_num--;			// client number decrease
		...
		for(i=0 ; i<idx ; i++){
			if(client_pid[i]!=0){
				printf("%d	", client_pid[i]);
				printf("%d	", client_port[i]);
				printf("%d \n", (int)cur_time-client_time[i]);	// current time - client_time arr of value
			}
		}
        printf("\n");
		return;
...
void print(struct data* struct_arr, int cnt)					// print function
{
		...
		if(sflag==1){
			...
			printf("111111111filename111111111111111111111111111111111\n");
		}	
}
