...
int ProcessNumber=5;
void *doit1(void* data);
void *doit2(void* data);
void *doit3(void* data);
void status_switch(int pid, int status);
/////////////////////////////////////////////////////
const char* logfile = "accessLog.log";	// log
int log_fd;					// file de
char log_buf[256] = {0,};	// log buffer
sem_t *mysem;
int main()
{	
	mysem = sem_open("40051", O_CREAT | O_RDWR, 0700, 1); // semaphore 1 set
	sem_close(mysem);
	log_fd= open(logfile, O_CREAT | O_TRUNC | O_RDWR, 0700);
	...
	////////////////////////////////////////////////////
	log_fd = open(logfile, O_CREAT | O_TRUNC | O_RDWR, 0666);
	//close(fd);
	...
	sprintf(log_buf, "[%s] Socket is created. Ip: %s, Port: %d\n", t_buf, inet_ntoa( *(struct in_addr*)host_entry->h_addr_list[0]), server_addr.sin_port);
	write(log_fd, log_buf, strlen(log_buf));
	//////////////////////////////////////////////////// socket binding
...
//////////////////////////////////////////////////// 
	...
	if(pid>0){			// parent part
		...
		sprintf(log_buf, "[%s] %d process is created. \n",t_buf ,getpid());
		write(log_fd, log_buf, strlen(log_buf));

		sprintf(log_buf, "[%s] IdleServerCount: %d\n",t_buf ,IdleServerCount);
		write(log_fd, log_buf, strlen(log_buf));
...
void child_main(int socketfd)
{
   ...
   while(1){
		...
		////////////////////////////////////////////////
		printf("============ New client ============\n");
		sprintf(log_buf, "============ New client ============\n");
		write(log_fd, log_buf, strlen(log_buf));
		printf("[%s] %s", t_buf, cwd);
		sprintf(log_buf, "[%s] %s", t_buf, cwd);
		write(log_fd, log_buf, strlen(log_buf));
		if(flag==0){
			printf(" %s\n", "403 Forbidden");
			sprintf(log_buf," 403 Forbidden\n");
		}
		else{
			printf(" %s\n", "200 OK");
			sprintf(log_buf," 200 OK\n");
		}		
		write(log_fd, log_buf, strlen(log_buf));
		...
		sprintf(log_buf, "IP: %s\n", client_ip);
		write(log_fd, log_buf, strlen(log_buf));
		printf("Port: %d\n", client_addr.sin_port);
		
		sprintf(log_buf, "Port: %d\n", client_addr.sin_port);
		write(log_fd, log_buf, strlen(log_buf));
		printf("PID: %d\n", getpid());
		
		sprintf(log_buf, "PID: %d\n", getpid());
		write(log_fd, log_buf, strlen(log_buf));
		printf("====================================\n");
		...
		sprintf(log_buf,"============ Disconnected client ============\n");
		write(log_fd, log_buf, strlen(log_buf));	
		printf("[%s] %s", t_buf, cwd);
		sprintf(log_buf, "[%s] %s", t_buf, cwd);
		write(log_fd, log_buf, strlen(log_buf));
				...
				if(MinSpareServers>IdleServerCount){// MinSpareServers condition
					for(i=0 ; i<j ; i++){					
						if(MaxChild==ProcessNumber){	// Maxchild condition
								...
								sprintf(log_buf,"[%s] Maximum exceed! Not Create child\n", t_buf);
								write(log_fd, log_buf, strlen(log_buf));	
								break;
						}					
...
void *doit3(void *vptr)	// tidC, CHILD THREAD DISCONNECT
{
	...
	mysem = sem_open("40051", O_RDWR); // opent
	///////////////////////////////////////////////////////////////////////////
	sem_wait(mysem);	// wait
...
void *doit4(void *vptr) 	
{
	mysem = sem_open("40051", O_RDWR);
	sem_wait(mysem);
	log_fd=open(logfile, O_RDWR|O_APPEND, 0666);
	write(log_fd, (char *)vptr, strlen((char*)vptr));
	vptr=NULL;
	close(log_fd);
	sem_post(mysem);
	sem_close(mysem);
	...
	if(sig==SIGINT){			// when is SIGINT, Parent part and child ignore!!
		...
		if(sigint==1){			// when is sigint==1
			...
			while(pHead!=NULL){	// all of 2D node delete
				...
				sprintf(log_buf,"[%s] %d process is terminated.\n", t_buf,pTemp->pid);
				write(log_fd, log_buf, strlen(log_buf));			
				printf("[%s] IdleServerCount: %d\n", t_buf, IdleServerCount);
			...
			write(log_fd, log_buf, strlen(log_buf));
			sprintf(log_buf,"[%s] server is terminated.\n", t_buf);
			write(log_fd, log_buf, strlen(log_buf));
			//////////////////////////////////////////////////////////////////////
			if((shm_id = shmget((key_t)KEY_NUM, MEM_SIZE, IPC_CREAT|0666)) == -1){
				printf("shmget fail\n");
